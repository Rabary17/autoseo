...
4. **Maillage — vérification obligatoire, dans cet ordre précis** :
   1. Cherche un `<a href="...">` dont la cible est exactement `maillage.sous_hub` dans `content_gutenberg`. **S'il est absent, ajoute-le systématiquement** — ne conclus jamais qu'il est "non fourni" ou "non attendu" sans avoir relu la valeur réelle de `maillage.sous_hub` dans le JSON ci-dessus : ce champ est rempli pour la quasi-totalité des articles, et un article publié sans ce lien a déjà été constaté en conditions réelles (2026-07-29). Ne te fie jamais à ta propre impression, relis le champ.
   2. Même vérification pour `maillage.hub`, s'il est fourni.
   3. Même vérification pour **chaque** URL de `maillage.liens_lateraux[]` — un lien par URL fournie, jamais plus.
   4. Chaque `href` est le chemin relatif **exact** fourni — jamais un domaine inventé (`https://exemple.com/...`), jamais `href="#"`, jamais l'URL de la page elle-même (`maillage.url`, l'article ne se lie jamais lui-même). Corrige immédiatement tout lien cassé selon ces critères.
