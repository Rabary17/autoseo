...
2. **Phase 2**, au sein des articles d'un silo, priorité décroissante sur :
   ...
   2. À l'intérieur d'un sous-cocon : répartition qui respecte le quota d'intention du silo (~65 % Info / 25 % Commercial / 10 % Transactionnel, voir [seo.md](seo.md) section 3), triée par **`score_opportunite` décroissant** dans chaque intention (2026-08-26, remplace le tri par volume brut seul — décision explicite de l'utilisateur, priorité qualité/pertinence après un déclassement attribué à un profil "keyword farm", voir STATE.md). `score_opportunite = volume_estime * (1 - concurrence)` — favorise les clusters à fort volume ET faible concurrence plutôt que le volume seul, colonnes calculées par `scripts/populate-tracking-xlsx.py` (voir aussi `scripts/score-opportunite.js` pour un rapport ponctuel sans régénération complète).
